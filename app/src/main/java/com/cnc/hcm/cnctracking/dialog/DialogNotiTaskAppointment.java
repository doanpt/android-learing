...
public class DialogNotiTaskAppointment extends Dialog implements View.OnClickListener {
    ...
    public void setData(String title, String timeAppointment) {
        if (tvTimeAppointment != null && tvTitleTask != null) {
            String timeItemTask = CommonMethod.formatTimeFromServerToString(timeAppointment);
            tvTitleTask.setText("Ticket " + title + " " + "còn 30 phút nữa đến lịch hẹn.");
            tvTimeAppointment.setText(timeItemTask);
        }
    }

    public interface OnButtonDialogAppointmentClickLister {
        void onButtonCancel();

        void onButtonViewTask();
    }
}
